...
def Param_Init(habit_num, price_num, social_num, random_state=None):
    ...
    from group-specific Beta distributions.
    ...
    Returns
    -------
    pd.DataFrame with one row per agent and columns:
    dominant_group, habit_str, price_sens, soc_suc
    """
    ...
    for group_name, count in mix.items():
        ...
        for i in range(count):
            ...
            agent = {
                ...
                "habit_str": rng.beta(
                    group_cfg['habit']['mu'] * group_cfg['habit']['K'],
                    (1 - group_cfg['habit']['mu']) * group_cfg['habit']['K']
                ),

                "price_sens": rng.beta(
                    group_cfg['price']['mu'] * group_cfg['price']['K'],
